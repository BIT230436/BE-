...
public class LeaveRequestService {
    ...
    /**
     * Intern hủy đơn nghỉ phép (chỉ khi đang ở trạng thái PENDING)
     */
    @Transactional
    public void cancelLeaveRequest(Long id) {
        LeaveRequest leaveRequest = leaveRequestRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy đơn nghỉ phép với ID: " + id));

        if (!"PENDING".equals(leaveRequest.getStatus())) {
            throw new IllegalStateException("Chỉ có thể hủy đơn đang chờ duyệt (PENDING)");
        }
